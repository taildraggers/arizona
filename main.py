"""Aggregates Arizona-located listings from every taildraggers.com
...
ones whose location ends in ", AZ".
...
This repo is a straight copy of the companion Alabama/Alaska repos'
approach, with the state filter swapped to "AZ".
"""
...
PAGE_TITLE = "Taildragger Ads in Arizona"
STATE_ABBR = "AZ"
...
_ARIZONA_LOCATION_RE = re.compile(r",\s*AZ\s*$", re.IGNORECASE)


def _is_arizona(location: str) -> bool:
    return bool(_ARIZONA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        arizona_rows = [row for row in rows if _is_arizona(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(arizona_rows)} in Arizona")
        all_listings.extend(arizona_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Arizona listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Arizona listings: {len(listings)}")
