...
Daily aggregator of Arizona-located taildragger classified listings,
...
[Alaska](https://github.com/taildraggers/alaska) repos' approach, with the
state filter swapped to "AZ".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, AZ` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Arizona"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/arizona/`
...
```html
<iframe
  src="https://taildraggers.github.io/arizona/"
  title="Taildragger Ads in Arizona"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_ARIZONA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
